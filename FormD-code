...
function createFolderIfNotFound(section, file){
  ...
  var status = checkFolderList(section, sub_folders);
  ...
  if(status[0]){
   // is status is true 
    ...
    var folder_out = folder.createFolder(section);
    Logger.log("folder \'" + section + "\' created in " + folder.getName()); 
    Logger.log("Folder to be used " + folder_out.getName());
  }
  return folder_out;  
...
function moveToDifferentFolder(section, file){
  ...
  var folder = createFolderIfNotFound(section, file)
  file.moveTo(folder)
}

function getCurrentFolder(){
  
  var active_form = FormApp.getActiveForm();
  var active_form_id = DriveApp.getFileById(active_form.getId());
  var current_folder = active_form_id.getParents()[0].getName();
...
//accesses the uploaded file
  ...
  moveToDifferentFolder(section, file); // move the file to specified folder
}
